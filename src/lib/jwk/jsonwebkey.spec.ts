...
const invalidIsJsonWebKeyData: any[] = [
  undefined,
  null,
  true,
  1,
...
describe('JSON Web Key', () => {
  ...
  describe('constructor', () => {
    ...
    it('should throw when providing both JSON Web Key Parameters "x5u" and "x5c".', () => {
      expect(() => Reflect.construct(JsonWebKey, [{ ...parameters, x5u: 'http://cert-url.com' }])).toThrowWithMessage(
        InvalidJsonWebKeyError,
        'Cannot have both "x5u" and "x5c" JSON Web Key Parameters.',
      );
    });

    it('should throw when providing one of JSON Web Key Parameters "x5t" or "x5t#S256" without providing one of JSON Web Key Parameters "x5u" or "x5c".', () => {
      const { x5c, ...params } = parameters;

      expect(() =>
        Reflect.construct(JsonWebKey, [{ ...params, x5t: 'tKL6B4sJGgaVlhGEUwchEOJTA50' }]),
      ).toThrowWithMessage(
        InvalidJsonWebKeyError,
      ...
      expect(() =>
        Reflect.construct(JsonWebKey, [{ ...params, 'x5t#S256': 'hRYOcUfJLh-Md6KzaOTdk9lv8Q8lhvZh6ucIqd3GaG0' }]),
      ).toThrowWithMessage(
        InvalidJsonWebKeyError,
        'Cannot have an X.509 Certificate Thumbprint without an X.509 Certificate Chain.',
      );
  ...
  describe('isJsonWebKeyParameters()', () => {
    it.each(invalidIsJsonWebKeyData)('should return false when the provided data is invalid.', (data) => {
      expect(JsonWebKey.isJsonWebKeyParameters(data)).toBeFalse();
    });

    it('should return true when the provided data is a valid JSON Web Key Parameters object.', () => {
      expect(JsonWebKey.isJsonWebKeyParameters(parameters)).toBeTrue();
    });
  });
});
