...
export class ECDSAJsonWebSignatureDigitalSignatureBackend extends JsonWebSignatureDigitalSignatureBackend {
   ...
   */
  public async sign(message: Buffer, jwk: EllipticCurveJsonWebKey): Promise<Buffer> {
    ...
    return await signAsync(this.hash, message, jwk.cryptoKey);
  }
   ...
   */
  public async verify(signature: Buffer, message: Buffer, jwk: EllipticCurveJsonWebKey): Promise<void> {
    this.validateJsonWebKey(jwk);

    const result = await verifyAsync(this.hash, message, jwk.cryptoKey, signature);
   ...
   * @throws {InvalidJsonWebKeyError} The provided JSON Web Key cannot be used by the JSON Web Signature Digital Signature Algorithm.
   */
  private validateJsonWebKey(jwk: EllipticCurveJsonWebKey): void {
