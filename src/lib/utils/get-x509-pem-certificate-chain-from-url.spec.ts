...
describe('getX509PemCertificateChainFromUrl()', () => {
  const rawPemCertificateChain: string =
    '-----BEGIN CERTIFICATE-----\n' +
  ...
  it('should throw when getting an error while calling the provided URL.', async () => {
    ...
    await expect(getX509PemCertificateChainFromUrl('https://certificate-url.com')).rejects.toThrowWithMessage(
      InvalidJsonWebKeyError,
      'Failed to parse the X.509 PEM Certificate Chain from the provided URL.',
    );
  });

  it('should throw when the URL returns an invalid X.509 PEM Certificate Chain.', async () => {
    ...
    await expect(getX509PemCertificateChainFromUrl('http://certificate-url.com')).rejects.toThrowWithMessage(
      InvalidJsonWebKeyError,
      'Failed to parse the X.509 PEM Certificate Chain from the provided URL.',
    );
  });

  it('should return the X.509 PEM Certificate Chain from the provided URL.', async () => {
    let pemCertificateChain!: string[];

    http.get = jest.fn().mockImplementationOnce((_, cb) => {
      const stream = new Stream();
      cb(stream);
      stream.emit('data', rawPemCertificateChain);
      stream.emit('end');
    });

    await expect(
      async () => (pemCertificateChain = await getX509PemCertificateChainFromUrl('http://certificate-url.com')),
    ).resolves.not.toThrow();

    expect(pemCertificateChain).toBeArrayOfSize(3);
    expect(pemCertificateChain).toSatisfyAll((pemCertificate) => rawPemCertificateChain.includes(pemCertificate));
  });
});
